...
void MkLiuSPH::Single_Step()
{
  ...
  if (LiuParam.summation_density)
    Norm_Density(); //Uni_Density(); //MK_Density();//Sum_Density();
  else
    Con_Density();
  ...
  Art_Repel(); // artificial repelling to give minimum spacing of particles
...
void MkLiuSPH::Art_Drag()
{
  ...
  for (i = 0; i < NTotal; i++)
  {
    par[i].ADDVXDt = -6 * 3.14 * par[i].Eta * radius * par[i].XVel;
    par[i].ADDVYDt = -6 * 3.14 * par[i].Eta * radius * par[i].YVel;
    par[i].ADDVZDt = -6 * 3.14 * par[i].Eta * radius * par[i].ZVel;
  }
...
void MkLiuSPH::LowDensity()
{
  ...
  for (i = 0; i < mp; i++)
  {
    ...
    {
      k = j + i * np;
      LiuParticles[k].X = rand() % 10000 / 20000.0 + 0.25; //(i)*dx/1.1 + dx/2.0;
      LiuParticles[k].Y = rand() % 10000 / 20000.0 + 0.25; //(j)*dy/1.1 + dy/2.0;
    }
  }
...
double MkLiuSPH::Calc_Norm()
{
  ...
  for (i = 1; i < np; i++)
  {
    par[i].X = hsml * cos(i * 3.14159 / 3);
    par[i].Y = hsml * sin(i * 3.14159 / 3);
    par[i].Z = 0;
...
int MkLiuSPH::Pair_Count()
{
  ...
  //MkDebug("Number of pair count %d\n",niac);//usleep(999999);usleep(999999);usleep(999999);
  return niac;
}
