...
void MkLiuSPH::Run()
{
  ...
  if (!is_first)
  {
    for (i = 0; i < NTotal; i++)
    {
      ...
      temp_u = 0.;
      if (Dim == 1)
        temp_u = -LiuParam.nsym * par[i].Press * par[i].XVel / par[i].X / par[i].Rho;
      ...
      if (!LiuParam.summation_density)
      {
        ...
        temp_rho = 0.;
        if (Dim == 1)
          temp_rho = -LiuParam.nsym * par[i].Rho * par[i].XVel / par[i].X;
...
void MkLiuSPH::Single_Step()
{
  ...
  if (LiuParam.summation_density)
    Norm_Bnd_Density(); //Uni_Density(); //MK_Density();//Sum_Density();
  else
    Con_Density();
...
void MkLiuSPH::Art_Repel()
{
  ...
  for (k = 0; k < NIac; k++)
  {
    ...
    if (dist > par[i].Radius + par[j].Radius)
      continue;
...
void MkLiuSPH::Art_Bnd_Repel()
{
  ...
  for (k = 0; k < NBIac; k++)
  {
    ...
    if (dist > par[i].Radius + bnd[j].Radius)
      continue;
...
void MkLiuSPH::Int_Force()
{
  ...
  for (i = 0; i < NTotal + NVirt; i++)
  {
    ...
    if (LiuParam.visc)
    {
      ...
      {
        par[i].TDSDt = txx(i) * txx(i) + tyy(i) * tyy(i) + tzz(i) * tzz(i) + 2.e0 * txy(i) * txy(i) + 2.e0 * txz(i) * txz(i) + 2.e0 * tyz(i) * tyz(i);
      }
  ...
  for (k = 0; k < NIac; k++)
  {
    ...
    if (LiuParam.pa_sph == 1)
    {
      ...
      for (d = 0; d < Dim; d++)
      {
        ...
        if (d == 0)
          he = he + (par[j].XVel - par[i].XVel) * h;
        if (d == 1)
          he = he + (par[j].YVel - par[i].YVel) * h;
        if (d == 2)
          he = he + (par[j].ZVel - par[i].ZVel) * h;
        ...
        if (LiuParam.visc)
        {
          ...
          else if (d == 1)
          {
            ...
            h = h + (par[i].Eta * txy(i) + par[j].Eta * txy(j)) * LiuPairs[k].DWDX(1 - 1) + (par[i].Eta * tyy(i) + par[j].Eta * tyy(j)) * LiuPairs[k].DWDX(2 - 1);
            if (Dim == 3)
            {
          ...
          else if (d == 2)
          {
            ...
            h = h + (par[i].Eta * txz(i) + par[j].Eta * txz(j)) * LiuPairs[k].DWDX(1 - 1) + (par[i].Eta * tyz(i) + par[j].Eta * tyz(j)) * LiuPairs[k].DWDX(2 - 1) + (par[i].Eta * tzz(i) + par[j].Eta * tzz(j)) * LiuPairs[k].DWDX(3 - 1);
          }
        }
        h = h * rhoij;
    ...
    else if (LiuParam.pa_sph == 2)
    {
      ...
      for (d = 0; d < Dim; d++)
      {
        ...
        if (LiuParam.visc)
        {
          ...
          {
            //   y-coordinate of acceleration
            h = h + (par[i].Eta * txy(i) / r2i + par[j].Eta * txy(j) / r2j) * LiuPairs[k].DWDX(1 - 1) + (par[i].Eta * tyy(i) / r2i + par[j].Eta * tyy(j) / r2j) * LiuPairs[k].DWDX(2 - 1);
            if (Dim == 3)
            {
              h = h + (par[i].Eta * tyz(i) / r2i + par[j].Eta * tyz(j) / r2j) * LiuPairs[k].DWDX(3 - 1);
            }
          }
          else if (d == 2)
          {
            //   z-coordinate of acceleration
            h = h + (par[i].Eta * txz(i) / r2i + par[j].Eta * txz(j) / r2j) * LiuPairs[k].DWDX(1 - 1) + (par[i].Eta * tyz(i) / r2i + par[j].Eta * tyz(j) / r2j) * LiuPairs[k].DWDX(2 - 1) + (par[i].Eta * tzz(i) / r2i + par[j].Eta * tzz(j) / r2j) * LiuPairs[k].DWDX(3 - 1);
          }
        }
...
void MkLiuSPH::Int_Bnd_Force()
{
  ...
  if (LiuParam.visc)
  {
    ...
    for (k = 0; k < NBIac; k++)
    {
      ...
      hzz = 2.e0 / 3.e0 * hzz;
      I = i;
      J = bnd.GetSize() + j; // particle I bnd J
      if (Dim == 1)
      {
  ...
  for (i = 0; i < bnd.GetSize(); i++)
  {
    ...
    if (LiuParam.visc)
    {
      ...
      {
        bnd[i].TDSDt = b_txx(i) * b_txx(i) + b_tyy(i) * b_tyy(i) + b_tzz(i) * b_tzz(i) + 2.e0 * b_txy(i) * b_txy(i) + 2.e0 * b_txz(i) * b_txz(i) + 2.e0 * b_tyz(i) * b_tyz(i);
      }
  ...
  for (k = 0; k < NBIac; k++)
  {
    ...
    if (LiuParam.pa_sph == 1)
    {
      ...
      for (d = 0; d < Dim; d++)
      {
        ...
        if (d == 0)
          he = he + (bnd[j].XVel - par[i].XVel) * h;
        if (d == 1)
          he = he + (bnd[j].YVel - par[i].YVel) * h;
        if (d == 2)
          he = he + (bnd[j].ZVel - par[i].ZVel) * h;
        ...
        if (LiuParam.visc)
        {
          ...
          {
            //   y-coordinate of acceleration
            h = h + (par[i].Eta * txy(I) + bnd[j].Eta * txy(J)) * LiuPairs[k].DWDX(1 - 1) + (par[i].Eta * tyy(I) + bnd[j].Eta * tyy(J)) * LiuPairs[k].DWDX(2 - 1);
            if (Dim == 3)
            {
          ...
          {
            //   z-coordinate of acceleration
            h = h + (par[i].Eta * txz(I) + bnd[j].Eta * txz(J)) * LiuPairs[k].DWDX(1 - 1) + (par[i].Eta * tyz(I) + bnd[j].Eta * tyz(J)) * LiuPairs[k].DWDX(2 - 1) + (par[i].Eta * tzz(I) + bnd[j].Eta * tzz(J)) * LiuPairs[k].DWDX(3 - 1);
          }
        }
        h = h * rhoij;
        //MkDebug("Int_Force i:%d, j:%d, h:%f, rhoij:%f\n",i,j,h,rhoij);
    ...
    else if (LiuParam.pa_sph == 2)
    {
      ...
      for (d = 0; d < Dim; d++)
      {
        ...
        if (LiuParam.visc)
        {
          ...
          {
            //   y-coordinate of acceleration
            h = h + (par[i].Eta * b_txy(I) / r2i + bnd[j].Eta * b_txy(J) / r2j) * LiuPairs[k].DWDX(1 - 1) + (par[i].Eta * b_tyy(I) / r2i + bnd[j].Eta * b_tyy(J) / r2j) * LiuPairs[k].DWDX(2 - 1);
            if (Dim == 3)
            {
              h = h + (par[i].Eta * b_tyz(I) / r2i + bnd[j].Eta * b_tyz(J) / r2j) * LiuPairs[k].DWDX(3 - 1);
            }
          }
          else if (d == 2)
          {
            //   z-coordinate of acceleration
            h = h + (par[i].Eta * b_txz(I) / r2i + bnd[j].Eta * b_txz(J) / r2j) * LiuPairs[k].DWDX(1 - 1) + (par[i].Eta * b_tyz(I) / r2i + bnd[j].Eta * b_tyz(J) / r2j) * LiuPairs[k].DWDX(2 - 1) + (par[i].Eta * b_tzz(I) / r2i + bnd[j].Eta * b_tzz(J) / r2j) * LiuPairs[k].DWDX(3 - 1);
          }
        }
  ...
  if (LiuParam.self_gravity)
  {
    for (i = 0; i < NTotal + NVirt; i++)
    {
      ...
      if (Dim == 2)
        par[i].EXDVYDt = -9.8;
      if (Dim == 3)
        par[i].EXDVYDt = 0; //-0.098; //par[i].EXDVZDt = -9.8;
    }
  }
...
void MkLiuSPH::Input()
{
  if (LiuParam.shocktube)
    Shock_Tube();
  else if (LiuParam.shearcavity)
    Shear_Cavity();
  else if (LiuParam.lowdensity)
    LowDensity();
}
...
{
  int i, j, ii, jj, niac = 0, scale_k;
  int I, J, K, II, JJ, KK, NX, NY, NZ;
  double dist, mhsml, dx, dy, dz;
  MkLiuParticles &par = LiuParticles;
  ...
  //MkDebug("\n PGC 01 NX %d NY %d ",LiuGrids.GetNX(),LiuGrids.GetNY());
  if (LiuParam.skf == 1)
    scale_k = 2;
  else
    scale_k = 3;
  ...
  for (I = 0; I < NX; I++)
  {
    for (J = 0; J < NY; J++)
    {
      for (K = 0; K < NZ; K++)
      {
        //MkDebug(" PGC I %d J %d  K %d   ",I,J,K);
        MkLiuGrid &grid = LiuGrids(I, J, K);
        //MkDebug(" PGC grid   ");
        MkInt &master = grid.GetParticleRef();
        //if (grid.GetNumOfParticle() <= 0) continue;
        for (ii = 0; ii < master.getSzX(); ii++)
        {
          if (-1 == (i = master[ii]))
          {
            continue;
          }
          for (II = max(I - 1, 0); II <= min(I + 1, NX); II++)
          {
            for (JJ = max(J - 1, 0); JJ <= min(J + 1, NY); JJ++)
            {
              for (KK = max(K - 1, 0); KK <= min(K + 1, NZ); KK++)
              {
                MkInt &slave = LiuGrids(II, JJ, KK).GetParticleRef();
                //if (LiuGrids(II,JJ,KK).GetNumOfParticle() <= 0) continue;
                for (jj = 0; jj < slave.getSzX(); jj++)
                {
                  if (-1 == (j = slave[jj]))
                    continue;
                  if (i >= j)
...
{
  int i, j, d, ii, jj, sumiac, maxiac, miniac, niac, noiac, maxp, minp, scale_k;
  int I, J, K, II, JJ, KK, NX, NY, NZ;
  double dx, dy, dz, dist, mhsml;
  MkLiuParticles &par = LiuParticles;
  ...
  NIac = Pair_Grid_Count();
  //MkDebug("Pair_Grid_Count %d, Pair_Count %d",NIac,Pair_Count());
...
int MkLiuSPH::Pair_Grid_Count_backup()
{
  int i, j, niac = 0, scale_k;
  int I, J, K, ii, jj;
  double dist, mhsml, dx, dy, dz;
  ...
  for (I = 0; I < LiuGrids.GetNX(); I++)
  {
    for (J = 0; J < LiuGrids.GetNY(); J++)
    {
      for (K = 0; K < LiuGrids.GetNZ(); K++)
      {
        ...
        //MkDebug(" PGC paref   ");
        MkInt &pr = grid.GetParticleRef();
        //MkDebug(" PGC referenced   ");

        int n = pr.getSzX();

        //MkDebug(" PGC sized n %d ",n);
        if (0 == n)
          continue;

        for (ii = 0; ii < n /*grid.GetParticleRef().getSzX()*/; ii++)
        {
          //MkDebug("PGC ii %d  ",ii);
...
void MkLiuSPH::Direct_Grid_Find_backup()
{
  int i, j, d, sumiac, maxiac, miniac, niac, noiac, maxp, minp, scale_k;
  int I, J, K, ii, jj;
  double dx, dy, dz, dist, mhsml;
  ...
  for (I = 0; I < LiuGrids.GetNX(); I++)
  {
    for (J = 0; J < LiuGrids.GetNY(); J++)
    {
      for (K = 0; K < LiuGrids.GetNZ(); K++)
      {
        ...
        if (LiuGrids.GetNumOfParticles(I, J, K) <= 0)
          continue;

        MkInt &paref = LiuGrids.GetParticles(I, J, K);
        //MkDebug("paref has particles %d\n",paref.getSzX());
        for (ii = 0; ii < grid.GetParticleRef().getSzX(); ii++)
        {
          for (jj = 0; jj < paref.getSzX(); jj++)
          {
            i = grid.GetParticleRef()[ii];
...
void MkLiuSPH::Viscosity()
{
  ...
  for (i = 0; i < NTotal + NVirt; i++)
  {
    if (par[i].GetParticleType() == 1)
      par[i].SetEta(0.0);
    else if (par[i].GetParticleType() == 2)
      par[i].SetEta(vis);
  }
}
