...
MkSparseMatrix::MkSparseMatrix(MkMatrix<float> &mat)
{
  SetMatrix(mat);
}
...
void MkSparseMatrix::SetMatrix(MkMatrix<float> &mat)
{
  int i, j;
  M = mat.GetFI();
  N = mat.GetFJ();
...
bool MkSparseMatrix::LUDecompose()
{
  ...
  {
    if (fabs(vv(i)) < EPS)
      MkDebug("Singular matrix in routine MkMatrix::LUDecompose");
    vv(i) = 1.0 / vv(i);
  }
  ...
  for (j = 0; j < n; j++)
  {
    for (i = 0; i < j; i++)
    {
      ...
      up = FindFirstInCol(j);
      if (!low || !up)
        continue; // if there is no element for low or up, no need to calculate
      ...
      if (fabs(sum) > TINY && is_exist)
        (*this)(i, j) = sum;
      else if (fabs(sum) > TINY && !is_exist)
        Add(i, j, sum);
    }
    big = 0.0;
    for (i = j; i < n; i++)
    ...
    if (j != imax)
    {
      ...
      while (smj)
      {
        if (smj->FI == j)
        {
          if (!IsExist(imax, smj->FJ))
            Move(smj, imax, smj->FJ);
        }
      }
...
bool MkSparseMatrix::LUBackSubstitute(MkVector<float> &b)
{
  int i, ii = -1, ip, j;
  double sum;
  MkFloat diag;
...
bool MkSparseMatrix::GaussSeidel(MkVector<float> &X0, MkVector<float> &B)
{
...
bool MkSparseMatrix::Solve(MkVector<float> &B)
{
  if (isSingular())
    return false;
  MkVector<float> X;
...
bool MkSparseMatrix::Solve(MkVector<float> &B, SolveType solve_type)
{
  if (isSingular())
    return false;
...
{
  if (FMatType == mtNormal)
    return addByNormal(sm);
  else if (FMatType == mtGauss)
    return addByGauss(sm);
  else if (FMatType == mtCol)
    return addByCol(sm);
  else if (FMatType == mtRow)
    return addByRow(sm);
  return false;
}
...
{
  if (FMatType == mtNormal)
    return delByNormal(sm);
  else if (FMatType == mtGauss)
    return delByGauss(sm);
  else if (FMatType == mtCol)
    return delByCol(sm);
  else if (FMatType == mtRow)
    return delByRow(sm);
...
{
  if (FMatType == mtNormal)
    return moveByNormal(sm, ai, aj);
  else if (FMatType == mtGauss)
    return moveByGauss(sm, ai, aj);
  else if (FMatType == mtCol)
    return moveByCol(sm, ai, aj);
  else if (FMatType == mtRow)
    return moveByRow(sm, ai, aj);
  return false;
}
...
bool MkSparseMatrix::IsExist(int ai, int aj)
{
  ...
  if (!Cur)
    return false; // if it happens, it is really strange!!!
  else
    return true;
}
...
{
  if (FMatType == mtNormal)
    return findFirstInRowByNormal(ai);
  else if (FMatType == mtGauss)
    return findFirstInRowByGauss(ai);
  else if (FMatType == mtCol)
    return findFirstInRowByCol(ai);
  else if (FMatType == mtRow)
    return findFirstInRowByRow(ai);
  return SMNull;
}
...
{
  if (FMatType == mtNormal)
    return findFirstInColByNormal(aj);
  else if (FMatType == mtGauss)
    return findFirstInColByGauss(aj);
  else if (FMatType == mtCol)
    return findFirstInColByCol(aj);
  else if (FMatType == mtCol)
    return findFirstInColByCol(aj);
  return SMNull;
}
...
{
  if (FMatType == mtNormal)
    return findLastInRowByNormal(ai);
  else if (FMatType == mtGauss)
    return findLastInRowByGauss(ai);
  else if (FMatType == mtCol)
    return findLastInRowByCol(ai);
  else if (FMatType == mtRow)
    return findLastInRowByRow(ai);
  return SMNull;
}
...
{
  if (FMatType == mtNormal)
    return findLastInColByNormal(aj);
  else if (FMatType == mtGauss)
    return findLastInColByGauss(aj);
  else if (FMatType == mtCol)
    return findLastInColByCol(aj);
  else if (FMatType == mtCol)
    return findLastInColByCol(aj);
  return SMNull;
}
...
int MkSparseMatrix::orderNormal(MkSMij *sm)
{
  ...
  if (I == J)
    ...
    return I * (I - 1) / 2 + J + 1 + n - 1;
  else if (I < J)
    return J * (J - 1) / 2 + I + 1 + (n - 1) * (n - 2) / 2 + 2 * (n - 1);
  return -1;
}
...
{
  if (FMatType == mtNormal)
    return orderNormal(sm1) > orderNormal(sm2);
  else if (FMatType == mtGauss)
    return orderGauss(sm1) > orderGauss(sm2);
  else if (FMatType == mtCol)
    return orderCol(sm1) > orderCol(sm2);
  else if (FMatType == mtRow)
    return orderRow(sm1) > orderRow(sm2);
  else
    return false;
}
...
bool MkSparseMatrix::addByNormal(MkSMij *sm)
{
  ...
  if (IsExist(sm))
  { // Find appropriate location to add
    delete sm;
    return false;
  }
...
bool MkSparseMatrix::delByNormal(MkSMij *sm)
{
  ...
  else if (sm == Low)
  {
    Low = sm->NextSM;
    if (Low)
      Low->PrevSM = sm->PrevSM;
    if (sm != NULL ? sm->PrevSM != NULL : false)
...
MkSMij *MkSparseMatrix::findFirstInRowByNormal(int ai)
{
  ...
  if (cur->FI == ai)
    return cur;
  else if (cur->NextSM ? cur->NextSM->FI == ai : false)
    return cur = cur->NextSM;
  else if (cur->PrevSM ? cur->PrevSM->FI == ai : false)
    return cur = cur->PrevSM;
  else
    return NULL;
}
...
MkSMij *MkSparseMatrix::findFirstInColByNormal(int aj)
{
  ...
  if (cur->FJ == aj)
    return cur;
  else if (cur->NextSM ? cur->NextSM->FJ == aj : false)
    return cur = cur->NextSM;
  else if (cur->PrevSM ? cur->PrevSM->FJ == aj : false)
    return cur = cur->PrevSM;
  else
    return NULL;
}
...
bool MkSparseMatrix::arrangeByGauss() // should be checked!
{
  if (FMatType == mtGauss)
    return true;
  else if (FMatType == mtLUDecomposed)
    return false;
  else if (FMatType == mtInverted)
    Invert();
  else if (FMatType == mtTransposed)
    Transpose();
  ...
  while (up)
  {
    ...
    while (low)
    {
      ...
      if (low->FI > up->FI)
        break;
      else if (low->FI == up->FI && low->FJ > up->FJ)
        break;
      else if (low->FI < up->FI && up->FI < next->FI)
        break;
      else if (low->FI == up->FI && up->FI < next->FI && low->FJ < up->FJ)
        break;
      else if (low->FI < up->FI && up->FI == next->FI && up->FJ < next->FJ)
        break;
      else if (low->FI == up->FI && up->FI == next->FI && low->FJ < up->FJ && up->FJ < next->FJ)
        break;
      low = low->Next();
    }
